...
static ngx_int_t
ngx_http_aws_proxy_header_exists(ngx_list_t *headers, u_char *name, size_t len)
{
    ngx_uint_t i;
    ...
    for (i = 0; /* void */; i++) {
        ...
        if (header[i].key.len == len && ngx_strcasecmp(header[i].key.data, name) == 0) {
            return 1; /* header exist */
        }
    }

    return 0; /* header non-exist */
}
...
ngx_http_aws_proxy_sign(ngx_http_request_t *r)
{
    ...
    for(i = 0; i < headers_out->nelts; i++)
    {
        ...
        if (ngx_http_aws_proxy_header_exists(&r->headers_in.headers, hv->key.data, hv->key.len)) {
            /*  If the same header field (case insensitive) already exists in r->headers_in.headers, skip it. */
            continue;
        }
