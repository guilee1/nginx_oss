...
static const ngx_str_t ACCEPT_HEADER = ngx_string("accept");
...
static inline const ngx_str_t* ngx_aws_auth__compute_request_timeStamp(ngx_pool_t *pool, const time_t *timep) {
	ngx_str_t *const retval = ngx_palloc(pool, sizeof(ngx_str_t));
	retval->data = ngx_palloc(pool, AMZ_DATE_MAX_LEN);
	struct tm *tm_p = ngx_palloc(pool, sizeof(struct tm));
#ifdef WIN32
  gmtime_s(tm_p,timep);
	...
	gmtime_r(timep, tm_p);
#endif  
	retval->len = strftime(__CHAR_PTR_U(retval->data), AMZ_DATE_MAX_LEN - 1, "%Y%m%d", tm_p);
	return retval;
}
...
//     ngx_array_t *settable_header_array = ngx_array_create(pool, 5, sizeof(header_pair_t));
//     header_pair_t *header_ptr;

//     ngx_list_part_t *part = (ngx_list_part_t *)&req->headers_in.headers.part;
...
//             i = 0;
//         }
// 		/* exclude accept and host
// 		*/
// 		if(ngx_strcasecmp(header[i].key.data, HOST_HEADER.data) == 0) {
//             continue;
//         }
// 		if(ngx_strcasecmp(header[i].key.data, ACCEPT_HEADER.data) == 0) {
//             continue;
//         }
		
//         header_ptr = ngx_array_push(settable_header_array);
// 		ngx_str_t lowercase_key;
//     	lowercase_key.len = header[i].key.len;
//     	lowercase_key.data = ngx_pnalloc(pool, header[i].key.len + 1);
// 		ngx_strlow(lowercase_key.data,  header[i].key.data,  header[i].key.len+1); 
// 		header_ptr->key = lowercase_key;
//         header_ptr->value = header[i].value;
//     }
...
//     header_ptr->value.len =  s3_endpoint->len + 1 ; //s3_bucket->len + s3_endpoint->len + 2;
...
static inline struct AwsCanonicalRequestDetails ngx_aws_auth__make_canonical_request(ngx_pool_t *pool,
	...
	retval.canon_request->len = ngx_snprintf(retval.canon_request->data, retval.canon_request->len, "%V\n/%V%V\n%V\n%V\n%V\n%V",
		http_method,s3_bucket_name,url, canon_qs, canon_headers.canon_header_str,
...
static inline const ngx_str_t* ngx_aws_auth__string_to_sign(ngx_pool_t *pool,
		const ngx_str_t *key_scope,	const ngx_str_t *date,const ngx_str_t *singleDateStamp, const ngx_str_t *canon_request_hash) {
	ngx_str_t *retval = ngx_palloc(pool, sizeof(ngx_str_t));
	...
	retval->len = ngx_snprintf(retval->data, retval->len, "AWS4-HMAC-SHA256\n%V\n%V/%V\n%V",
		date, singleDateStamp, key_scope, canon_request_hash) - retval->data ;
...
static inline const ngx_str_t* ngx_aws_auth__make_auth_token(ngx_pool_t *pool,
	...
	const ngx_str_t *access_key_id, const ngx_str_t *key_scope,const ngx_str_t *singleDateStamp) {

    const char FMT_STRING[] = "AWS4-HMAC-SHA256 Credential=%V/%V/%V,SignedHeaders=%V,Signature=%V";
	ngx_str_t *authz;
    ...
    authz->len = ngx_snprintf(authz->data, authz->len, FMT_STRING,
		access_key_id,singleDateStamp, key_scope, signed_header_names, signature) - authz->data;
	return authz;
}

ngx_str_t* ngx_aws_auth__sign_hmac_sha256_hex(ngx_pool_t *pool, const ngx_str_t *blob,
    const ngx_str_t *signing_key, const ngx_str_t  *singleDateStamp) {
    ...
    // get signingKey
    char signingKey[32];
    char kSecret[] = {65, 87, 83, 52, 49, 57, 51, 48, 54, 67, 53, 66, 51, 55, 66, 55, 52, 55, 50, 50, 56, 69, 66, 54, 65, 70, 48, 57, 51, 56, 68, 48, 50, 51, 50, 56, 55, 54, 50, 51};
    char kDate[32];
    Hmacsha256 ((unsigned char *) kSecret, 40, singleDateStamp->data, 8, (unsigned char *) kDate);
    char kRegion[32];
    Hmacsha256((unsigned char *) kDate, 32, (unsigned char *) "us-east-1", 9, (unsigned char *) kRegion);
    char kService[32];
    Hmacsha256( (unsigned char *) kRegion, 32, (unsigned char *) "s3", 2, (unsigned char *) kService);
    Hmacsha256( (unsigned char *) kService, 32, (unsigned char *) "aws4_request", 12, (unsigned char *) signingKey);
    char signature[32];
    Hmacsha256((unsigned char *) signingKey, 32, blob->data, blob->len,  (unsigned char *) signature);
    
	retval->data = ngx_palloc(pool, 65);
	retval->len = 64;
    hex((unsigned char *)signature, (char *)retval->data, 32);
	// ngx_hex_dump(retval->data, md, 32);
	return retval;
}
...
static inline struct AwsSignedRequestDetails ngx_aws_auth__compute_signature(ngx_pool_t *pool, ngx_http_request_t *req,
    ...
    const ngx_str_t *singleDateStamp) {
	...
	// get string to sign
	const ngx_str_t *string_to_sign = ngx_aws_auth__string_to_sign(pool, key_scope, date,singleDateStamp, canon_request_hash);
  safe_ngx_log_error(req, "string_to_sign is %V", string_to_sign);
	// generate signature
	const ngx_str_t *signature = ngx_aws_auth__sign_hmac_sha256_hex(pool, string_to_sign, signing_key, singleDateStamp);
	safe_ngx_log_error(req, "signature is %V", signature);
...
static inline const ngx_array_t* ngx_aws_auth__sign(ngx_pool_t *pool, ngx_http_request_t *req,
  ...
  const ngx_str_t *date = ngx_aws_auth__compute_request_time(pool, &req->start_sec);
	const ngx_str_t *singleDateStamp = ngx_aws_auth__compute_request_timeStamp(pool, &req->start_sec);
	const struct AwsSignedRequestDetails signature_details = ngx_aws_auth__compute_signature(pool, req, signing_key, key_scope, s3_bucket_name, s3_endpoint,date,singleDateStamp);


	const ngx_str_t *auth_header_value = ngx_aws_auth__make_auth_token(pool, signature_details.signature,
											signature_details.signed_header_names, access_key_id, key_scope,singleDateStamp);
